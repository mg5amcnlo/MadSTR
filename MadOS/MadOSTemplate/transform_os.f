...
C-----triangular function
      ...
      subroutine transform_os_genps(p,q,ip,jp,mass_i,mass_j,mass_ij,mom_perm,stat)
************************************************************************
*     Authors: Marco Zaro                                              *
*     after rescaling the center-of-mass energy so that the reshuffling*
*     can be done, it uses a reshuffling as described in sect 5.2 of   *
*     0709.2092                                                        *
*     Reshuffling routines have been coded by Rikkert Frederix         *
************************************************************************
      implicit none 
      include 'nexternal.inc'
C-----Arguments      
...
C-----Local
      ...
      double precision mreco, memitt
      double precision a, b
      double precision dot, threedot, sumdot
      external dot, threedot, sumdot
      double precision rescale_init
      double precision pmass(nexternal), sum_mass, ximass
      !changing ximass may lead to catastrophic failures
      parameter (ximass = 1d0) 
      double precision p_shat(0:3, nexternal)

      include 'coupl.inc'
      include 'run.inc'
      double precision zero
      parameter (zero=0d0)
! variables to be passed to the subroutine that puts momenta on-shell
      integer next
      parameter (next=2*nexternal-3)
      double precision p_os_in(0:4,next), p_os_out(0:4,next)
      integer decay_tree(2,-next:-1,2)
      integer i_os, skip
...
C the sum of all the other masses (except for those in the reshuffling
      sum_mass = 0d0
      do i = nincoming+1, nexternal
C        remember thatthe order of particles in the FS of the OS 
C        counterterm may differ from the one in the rest of the code
        if (mom_perm(i).ne.ip.and.mom_perm(i).ne.jp)
     $   sum_mass = sum_mass + pmass(i)
      enddo

C rescale shat (keep it below the collider energy) ; 
      rescale_init=(sum_mass+mass_ij)/(sum_mass+min(ximass*mass_ij,dsqrt(dot(pij,pij))))
      shat=min(shat*rescale_init**2, (ebeam(1)+ebeam(2))**2*0.99d0)

      if (dsqrt(shat).lt.sum_mass+mass_ij) then
        write(*,*) 'ERROR IN TRANSFORM_OS_GENPS: INCONSITENT SHAT',
     $   dsqrt(shat),sum_mass+mass_ij
        stop 1
      endif

      if (rescale_init.eq.1d0) then
        ! if shat is the same, just copy the momenta
        do i = 1, nexternal
          do nu = 0, 3
            p_shat(nu, i) = p(nu, i)
          enddo
        ...
        ! otherwise, call the PS generation routine to generate the momenta 
        !  with the new shat
        call generate_momenta_shat(shat,mom_perm,p_shat)
      endif

      if (p_shat(0,1).le.0d0) then
        write(*,*) 'RETURNING 1'
        stat=1
        return
      endif
...
C boost the momenta in the partonic C.o.M. frame
      do nu=0,3
        pboost(nu) = p_shat(nu,1) + p_shat(nu,2)
      enddo
      do i=1,nexternal
        call invboostx(p_shat(0,i), pboost, pcom(0,i))
      enddo

      ! use the OS_resonance_and_decay_reshuffle function to put stuff
      ! on-shell

      ! first, copy the argunment
      ! input momentum : copy all particles but ip and jp, which will 
      ! be put last
      i_os = 0
      do i = 1, nexternal
        ...
        i_os = i_os + 1
        do j = 0, 3 
          p_os_in(j, i_os) = pcom(j, i)
        enddo
        ! it is perhaps a dirty solution, but should be ok
        p_os_in(4,i_os) = dsqrt(max(dot(pcom(0,j),pcom(0,j)),0d0))
      enddo

      ! safety check
      if (i_os.ne.nexternal-2) then
        write(*,*) 'ERROR IN TRANSFORM_OS_GENPS: INCONSITENT I_OS',
     $     i_os, nexternal - 2
        stop 1
      endif

      ! then, add in order, the resonance (i+j), pi and pj
      do nu = 0, 3
        p_os_in(nu, nexternal - 1) = pcom(nu,ip) + pcom(nu,jp)
        p_os_in(nu, nexternal) = pcom(nu,ip)
        p_os_in(nu, nexternal + 1) = pcom(nu,jp)
      enddo
      p_os_in(4, nexternal - 1) = dsqrt(sumdot(pcom(0,ip), pcom(0,jp), 1d0))
      p_os_in(4, nexternal) = mass_i
      p_os_in(4, nexternal + 1) = mass_j

      ! finally, the decay tree
      decay_tree(1, -1, 1) = nexternal
      decay_tree(2, -1, 1) = nexternal + 1
      decay_tree(1, -1, 2) = nexternal
      decay_tree(2, -1, 2) = nexternal + 1

CC      subroutine OS_resonance_and_decay_reshuffle(npart,ibeta,decay_tree
CC     $     ,Mbeta,p_os_in,p_os_out)
      !  X  npart=nexternal (it is the number of particles before the
      !           extra emission, so neternal-1 + the resonance
      !  X  ibeta=nexternal -1 (the position of the resonance in p_os_in)
      !  X  mbeta=mass_ij
      call OS_resonance_and_decay_reshuffle(nexternal,nexternal-1,decay_tree
     $     ,mass_ij,p_os_in,p_os_out)

      ! copy back from p_os_out to qcom
      i_os = 0
      do i = 1, nexternal
        if (i.eq.ip.or.i.eq.jp) cycle
        i_os = i_os + 1
        do j = 0, 3 
          qcom(j, i) = p_os_out(j, i_os)
        enddo
      enddo
      ! as before, i is in p_os_out at position nexternal, j at nexternal+1 
      do j = 0, 3 
        qcom(j, ip) = p_os_out(j, nexternal)
        qcom(j, jp) = p_os_out(j, nexternal + 1)
      enddo

      ! finally boost back to the lab-frame
      do i=1, nexternal
...
C-----Arguments      
      ...
      subroutine generate_momenta_shat(shat, mom_perm, p_reord)
C a wrapper to the standard generate_momneta subroutine
      implicit none
      include 'nexternal.inc'
      double precision shat
      integer mom_perm(nexternal)
      ...
      double precision p_dum(0:3,nexternal)
      include 'mint.inc'
      ! stuff for the on-shell reshuffling
      double precision x(99)
      common /to_reshuffle/x
      integer         ndim
      common/tosigint/ndim
      integer i, j
      logical force_shat
      double precision shat_genps
      common/to_shat_genps/force_shat, shat_genps
...
! switch flags for soft/coll tests off in any case
      logical softtest,colltest,softtest_save,colltest_save
      common/sctests/softtest,colltest

      double precision shat_target, shat_in_up, shat_in_down
      double precision shat_out_up, shat_out_down, shat_out_half
      integer icount
      double precision shat_tolerance
      parameter(shat_tolerance=1d-8)

      shat_target=shat
      ! reduce shat when j is initial state
      if (j_fks.lt.nincoming) shat_target=shat*(1d0-xi_i_fks_ev)
      shat_genps=shat_target

      force_shat=.true.
      colltest_save=colltest
      softtest_save=softtest
      colltest=.false.
      softtest=.false.

      call generate_momenta(ndim,iconfig,wgt,x,p)
      ! this loop is necessary because, the phase space always generate
      ! the born shat, and in the case of ISR (j<=2) the real-emission
      ! shat also depends on xi_fks, which also changes (changing shat_born
      ! changes the boundaries of xi)
      ! normally, just a couple (2) iterations are enough to get shat
      ! accurage at less than 1e-8 relative accuracy 
      do while (abs(shat-sumdot(p(0,1),p(0,2),1d0))/shat.gt.
     $            shat_tolerance)
        shat_genps = shat_genps*shat/sumdot(p(0,1),p(0,2),1d0)
        call generate_momenta(ndim,iconfig,wgt,x,p)
      enddo

C remember to reorder the momenta, as the order of particles in the FS
C of the OS counterterm may differ from the one in the rest of the code
      do j = 1, nexternal
        do i = 0, 3
      ...
      ! the second call is to avoid border effects
      force_shat=.false.
      colltest=colltest_save
      softtest=softtest_save
      call generate_momenta(ndim,iconfig,wgt,x,p_dum)
      ! also call set_cms_stuff
